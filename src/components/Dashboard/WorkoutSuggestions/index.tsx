...
export const WorkoutSuggestions = ({
  ...
  const displayedSuggestions = showAllSuggestions ? localSuggestions : localSuggestions.slice(0, 3);
  return <div className="space-y-4">
      
      
      <SuggestionsGrid suggestions={displayedSuggestions} onSelectWorkout={handleSelectWorkout} />
      
      {/* Réduit en importance */}
      {showAllSuggestions && <SportProgramsSection />}
      
      <GenerateWorkoutDialog isOpen={isGenerateOpen} onClose={() => setIsGenerateOpen(false)} workoutType={selectedType} />
    </div>;
};