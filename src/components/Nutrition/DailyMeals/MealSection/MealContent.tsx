...
export const MealContent = ({ mealEntries, generatedMeal, onMealStatus }: MealContentProps) => {
  ...
  const calculatePercentage = (value: number, mealType: string) => {
    // Get the target values for this specific meal type
    const mealTarget = mealPlan[mealType];
    if (!mealTarget) return 0;
    
    return Math.round((value / mealTarget.calories) * 100);
  };
